...
# ============================================
# ETL ПРОЦЕССЫ - ЧАСТОТА ЗАПУСКА (НОВОЕ)
# ============================================
# Все интервалы указываются в минутах

# Загрузка клиентов (по умолчанию каждую минуту)
ETL_CLIENTS_INTERVAL=1

# Инкрементальная загрузка консультаций (новые и измененные по дате создания/консультации)
# Рекомендуется: 5-10 минут (по умолчанию 5)
ETL_CONS_INCREMENTAL_INTERVAL=5

# Обновление открытых консультаций по Ref_Key (все открытые заявки из БД)
# Рекомендуется: 15-30 минут (по умолчанию 30)
ETL_CONS_OPEN_UPDATE_INTERVAL=30

# Загрузка переносов консультаций (по умолчанию каждую минуту)
ETL_CONS_REDATE_INTERVAL=1

# Загрузка оценок консультаций (по умолчанию каждую минуту)
ETL_CONS_RATES_INTERVAL=1

# Загрузка дозвонов (по умолчанию каждую минуту)
ETL_CALLS_INTERVAL=1

# Загрузка закрытия очереди (по умолчанию каждую минуту)
ETL_QUEUE_CLOSING_INTERVAL=1

# Загрузка пользователей (менеджеров) - ежедневно
# Час запуска (по умолчанию 3:00 UTC)
# Для отключения установите ETL_USERS_CRON_HOUR=-1
ETL_USERS_CRON_HOUR=3
# Минута запуска (по умолчанию 15)
ETL_USERS_CRON_MINUTE=15

# ВАЖНО: Для отключения любого ETL процесса установите его интервал в 0
# Например: ETL_CONS_RATES_INTERVAL=0 отключит загрузку оценок

# ============================================
# ETL ПРОЦЕССЫ - ДОПОЛНИТЕЛЬНЫЕ НАСТРОЙКИ
# ============================================
# Размер страницы для пагинации OData запросов (по умолчанию 1000)
ODATA_PAGE_SIZE=1000

# Буфер для инкрементальной загрузки консультаций (дни назад от последней синхронизации)
# Рекомендуется: 7 дней (по умолчанию 7)
ETL_CONS_INCREMENTAL_BUFFER_DAYS=7

# Размер батча для запросов по Ref_Key при обновлении открытых консультаций
# Рекомендуется: 50-100 (по умолчанию 50)
ETL_CONS_REF_KEY_BATCH_SIZE=50

# Максимальное количество ошибок для логирования в одном запуске ETL
# После этого ошибки подавляются чтобы не создавать шум в логах (по умолчанию 10)
ETL_MAX_ERROR_LOGS=10
